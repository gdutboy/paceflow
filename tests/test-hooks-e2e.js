...
test('2d. SessionStart 超大注入会在 50KB 前截断并给路径化提示', () => {
  ...
  // artifact group：task.md section 先注入，截断发生在其内容内部
  assert.ok(r.stdout.includes('=== task.md ==='), 'artifact group 下 task.md section 应先注入');
...
test('2e3. SessionStart walkthrough 表格截断同日多条保留最新（prepend 最新在顶）', () => {
  ...
  const r = runHook('session-start.js', { cwd: dir, stdin: { type: 'startup' }, args: ['--group', 'artifact'] });
  assert.strictEqual(r.code, 0);
  // 11 条同日 → 保留最新 10（smoke 11~02），省略最旧 1（smoke 01）
  assert.ok(r.stdout.includes('smoke 11'), '最新条目 smoke 11（prepend index0）必须保留');
  assert.ok(r.stdout.includes('smoke 02'));
