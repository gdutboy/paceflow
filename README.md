...
- **SessionStart** — 会话启动时注入项目上下文，重置防循环计数器
...
├── README.md                                    # 本文件
├── hooks/                                       # Hook 脚本（5 个）
│   ├── pace-utils.js                            #   公共工具函数（多信号检测、文件读取）
│   ├── pre-tool-use.js                          #   PreToolUse：三级触发 + C 阶段批准 + Write 保护
│   ├── post-tool-use.js                         #   PostToolUse：归档提醒 + findings 检查 + Tasks API 同步
...
| 级别 | 条件 | 动作 |
|------|------|------|
| **Deny** | 强信号 + 无活跃任务 | `permissionDecision: "deny"` |
| **Deny** | 3+ 代码文件 + 无活跃任务 | `permissionDecision: "deny"` |
...
| Stop | - | stderr + exit 2 | `exit 2`（stderr 反馈给 AI）|
...
| 版本 | 日期 | 主要变更 |
|------|------|----------|
| v4.3.4 | 2026-02-14 | ticket4 审查修复（try-catch / 正则 / 阈值 / findings 降级 / Tasks API 同步 / .pace/ 目录）|
