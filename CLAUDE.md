...
**Corrections 捕获**：被用户纠正时（"不对"、"别这样"、"我说的是"、"错了"等），必须派 `artifact-writer record-correction` 写入 `changes/corrections/<id>.md` 与 `corrections.md` 索引（错误行为 + 正确做法 + 触发场景 + 根本原因）。禁止仅口头承认而不持久化。
...
1. 派 `artifact-writer update-chg action=update-status` 确认详情文件所有 T-NNN 为 `[x]` 或 `[-]`，frontmatter `status: completed`（若最后收口时仍有开放任务，`close-chg complete-open-tasks: true` 也可一次收口）
2. 运行验证并读取结果；确认通过后派 `close-chg verification-confirmed: true`，写 `verified-date` + `<!-- VERIFIED -->`，移动 `task.md` / `implementation_plan.md` 索引到 ARCHIVE 下方，并写 `walkthrough.md`
3. 若只需单独记录 V 阶段而暂不归档，才派 `update-chg action=verify`
4. 必要时 `spec.md` 同步技术栈变更
