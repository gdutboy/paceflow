...
**Subagent 分流**：研究/探索/搜索任务（预计 5+ 次搜索/读取，如 Grep/Glob/Read/WebSearch，或 native build 上的 Bash `rg`/`find` fallback）优先分流到 subagent，一个 subagent 一个明确任务，保持主上下文窗口干净。定向搜索（找特定文件/函数/类）可直接在主上下文执行。
...
**豁免**：问答、单行修改、纯文档/注释、纯重构、用户明确说"简单改一下"
...
**启用后**：先读取 `pace-workflow` Skill。涉及 artifact 字段、任务状态、批准、验证或归档时，再读取 `artifact-management` Skill。

**不可绕过**：
- artifact CRUD 统一派 `paceflow:artifact-writer`，主 session 不直接写 `task.md` / `implementation_plan.md` / `walkthrough.md` / `findings.md` / `corrections.md` / `changes/**`
- 写 `APPROVED` / `VERIFIED` / `verified-date` 前必须走 skill 中的结构化 operation 模板
- `[ ]` ready 与 `[!]` blocked/deferred 允许 Stop 提醒，但不是项目文件写入许可；必须进入 `[/]` running 才能执行项目修改

---

## G-9. 任务完成检查

每个 CHG/HOTFIX 的最后一个任务代码写完后，立即执行：

1. 运行验证并读取结果；验证结果没读完前，不派 `verify` 或 `close-chg`
2. 验证通过后优先按 `artifact-management` 的 `close-chg` 模板一次收口并归档
3. 必要时同步 `spec.md` 中的技术栈、依赖、配置或目录约定变化
