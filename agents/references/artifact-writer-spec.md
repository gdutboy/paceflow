...
5. 写入路径：派 `artifact-writer` 执行 `update-chg action=verify`，或在收尾时执行 `close-chg`（详见 `${CLAUDE_PLUGIN_ROOT}/agents/references/instructions/update-chg.md` / `close-chg.md`）
...
| `<!-- APPROVED -->` | 单 CHG C 阶段批准 | 详情文件 `changes/<id>.md` | 派 `update-chg action=approve` 或 `approve-and-start` |
