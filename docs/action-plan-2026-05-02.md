...
node tests/test-hooks-e2e.js                         # 236/236 PASS
node tests/test-pace-utils.js                        # 154/154 PASS
...
| P1-UX | 旧 Superpowers plan 与 TaskSync 不可见提醒噪声 | ✅ 已修复。Windows `paceflow` installed runtime 观察到：很久以前的 `docs/plans/*.md` 因未写 `.pace/synced-plans` 仍被当作当前 bridge 信号；`TaskUpdate` 会反复把 bridge 提醒作为 `additionalContext` 注入给 Claude，但用户在 TUI 不可见 | `formatBridgeHint()` 只返回近期或 `.pace/current-native-plan` 显式选中的 plan；旧 plan 降级为历史 backlog。`TaskCreate/TaskUpdate/TodoWrite` 在非 artifact 场景 log-only，不再持续注入用户不可见提醒。真正写代码或派 artifact-writer 时仍由 PreToolUse hard gate 提醒 bridge |
...
- 回归验证：新增 `9haa0` / `9haa0b` 覆盖无 PACE 信号的显式 artifact-writer 入口；当前基线为 `node tests/test-hooks-e2e.js` 236/236 PASS，`node tests/test-pace-utils.js` 154/154 PASS；`node tests/test-install.js` 26/26 PASS 为本地 ignored manual-install tooling 补充检查。
