...
- Anthropic `Prompting best practices`：Claude Opus 4.7 字面执行、正向示例、scope 明确、tool use 显式触发
- Claude Code v2.1.139 changelog / hooks reference：新增 hook `args: string[]` exec form 与 `PostToolUse continueOnBlock` 配置
- Claude Code v2.1.141-v2.1.143 changelog / tools reference：`terminalSequence`、PowerShell tool 默认面扩大、Monitor 权限面、plugin projected context cost、Stop 连续 block cap
...
| ID | 状态 | 任务 | 问题 | 改动范围 | 验证 |
|---|---|---|---|---|---|
| P1-20260506-01 | ✅ v6.0.11 / v6.0.59 收敛 | SessionStart 执行上下文改为读取 CHG 详情任务统计 | 提示不应只用 `task.md` 索引 checkbox 判断执行状态；v6 子任务权威是 `changes/<id>.md ## 任务清单`，Claude 任务面板只是工作记忆 | `hooks/session-start.js`、`tests/test-hooks-e2e.js` | 有详情 pending T-NNN 时提示 CHG 执行上下文；仅索引无详情任务时不夸大 |
...
| P2-POC-07 | ✅ 代码迁移完成，installed smoke 待 2.1.139+ 复核 | v2.1.139 `hooks[].args` exec form 对稳定性有直接价值：避免 shell 字符串中 `${CLAUDE_PLUGIN_ROOT}`、Windows 路径、空格路径和引号转义问题 | `plugin/hooks/hooks.json` 与手动安装 excerpt 已改为 `command:"node" + args:[".../hook.js"]`；`claude plugin validate ./plugin`、安装测试与 e2e 已覆盖配置结构。真实 installed runtime 仍建议在 Claude Code 2.1.139+ 上做一次 smoke，确认 placeholder 在 `args` 中展开和 stdin/exit/stdout 语义一致 |
| P2-POC-08 | ✅ v6.0.57 `PostToolUse continue:true` 最小试点 | Smoke15 已确认 command hook 可输出 `decision:"block"` 且 `continue:true` 继续同一 turn；生产实现只迁移 walkthrough wikilink/context 终态修复 | 保持窄口径：每 session/目标 one-shot；第二次降级为 additionalContext；后续扩展只考虑 VERIFIED/frontmatter 等 PostToolUse 终态修复，不迁移 PreToolUse gate |
...
本地手动安装工具可选检查：`install.js`、`verify.js` 与 `tests/test-install.js` 是 ignored local tooling，不属于 tracked release gate；若本地副本存在，可额外跑 `node tests/test-install.js`。
...
本机已复核到 `2.1.141`；2026-05-14 已将 hook 注册迁移为 `args: string[]` exec form，`claude plugin validate ./plugin` 在当前本机通过，单元/e2e 覆盖配置结构。2026-05-15 在 Claude Code `2.1.141` installed runtime 下完成 Smoke10/11：SessionStart、PreToolUse、SubagentStop、Stop 均正常触发，说明 `args` 中 plugin placeholder 展开与 stdin/exit/stdout 语义在当前环境可用。2026-05-16 已基于官方 `2.1.143` 文档补齐 PowerShell/Monitor 工具面单元/e2e 覆盖；真实 Windows installed runtime 仍建议升级 2.1.143 后跑 Smoke14。`PostToolUse continue:true` 已先在 Smoke15 PoC 验证，再于 v6.0.57 限定迁移到 walkthrough 终态修复。
...
| 能力 | v6 权威行为 | 覆盖 |
|---|---|---|
| SessionStart startup/resume 注入 | 注入 v6 根索引活跃区、活跃 CHG 摘要、Artifact 目录、CHG 执行上下文、相关 thoughts/knowledge、Git 状态；首次 artifact-root 未选择时不主动打扰闲聊且不创建 `.pace` 或 Obsidian 空项目 | `test-hooks-e2e.js` 1/2/2a/2b/2c/9c3/9c3a/9c3b |
...
| PreCompact 前快照 | compact 前记录 activeChanges、task/implementation_plan 活跃状态、runtime blockCount/degraded/legacyTaskPanelUsed、findings openCount、walkthrough 今日记录、近期 native plan | `test-hooks-e2e.js` 19 |
...
| 优先级 | 缺口 | 当前状态 | 下一步 |
|---|---|---|---|
| P1 | Installed-plugin production smoke | ✅ v6.0.54 Smoke1-6 已复跑并记录；worktree walkthrough context 已补齐。v6.0.55 后续 Smoke10/11 在 Claude Code 2.1.141 installed runtime 下验证通过：缺字段 Agent prompt 在 PreToolUse 阶段被拒绝并返回完整模板，malformed index 修复后 create/happy path 正常闭环 | 发新版或改 hook/agent runtime 时按 `docs/production-smoke-v6.0.55.md` 复跑：root-choice vault/local helper、current-owner README/文档 C/E gate、foreign fresh worktree 并发、close/archive owner closed、prompt template gate、record-finding artifact_dir 模板 |
...
| P2-hardening | Claude Code v2.1.139 hook runtime modernization | ✅ `args: string[]` exec form 已完成代码迁移并通过 `claude plugin validate` / e2e；2026-05-15 在 Claude Code 2.1.141 installed runtime 下完成 Smoke10/11，确认 plugin placeholder 在 args 中展开，hook stdin/exit/stdout 语义正常。2026-05-16 Smoke15 隔离 PoC 已确认 command-type PostToolUse 可用 `decision:"block" + continue:true` 把 reason 喂回 Claude 并继续同一 turn | exec-form 已从待验证项移除；v6.0.57 仅把 continue block 用于 walkthrough 终态修复。禁止迁移 artifact-root 选择、legacy migration、C/E gate、Agent 缺字段、直接 artifact mutation、Bash/PowerShell runtime-control 写入等 PreToolUse gate |
| P1-hardening | PowerShell / Monitor 工具面写保护覆盖 | ✅ v6.0.56 已实现。Claude Code `PowerShell` 是独立工具名；2.1.143 已扩大 Windows 默认启用面。当前版本已在 `PreToolUse` 注册 `PowerShell` / `Monitor`，避免 PowerShell 或后台 Monitor 命令绕过 Bash artifact guard | 已补 `PowerShell` matcher 与 PowerShell 命令写保护：`Set-Content` / `Add-Content` / `Out-File` / `Remove-Item` / `Move-Item` / `Copy-Item` / redirection / alias / `node -e` / `.ps1` wrapper；Monitor 使用 Bash-like guard。新增 e2e，仍建议 Windows installed Smoke14 复核真实 tool payload |
| P1-UX | 旧 Superpowers plan 与 TaskSync 不可见提醒噪声 | ✅ 已修复。Windows `paceflow` installed runtime 观察到：很久以前的 `docs/plans/*.md` 因未写 `.pace/synced-plans` 仍被当作当前 bridge 信号；任务面板 hook 会反复把 bridge 提醒作为 `additionalContext` 注入给 Claude，但用户在 TUI 不可见 | `formatBridgeHint()` 只返回近期或 `.pace/current-native-plan` 显式选中的 plan；旧 plan 降级为历史 backlog。当前官方 hook 文档已将任务面板改为 `TaskCreated` / `TaskCompleted` 事件且无 matcher；PACEflow 不再注册任务面板 hook。真正写代码或派 artifact-writer 时仍由 PreToolUse hard gate 提醒 bridge |
| P2-POC | `PostToolUse continue:true` 即时修复循环 | ✅ v6.0.57 已落地最小生产试点。`/mnt/k/AI/cc-smoke15` 的隔离 PoC 先证明 command hook 可用 `decision:"block" + continue:true`；生产实现只覆盖 artifact-writer 写 `walkthrough.md` 后的 wikilink/context 终态修复，并使用 `post-continue-*` session flag one-shot 限流 | 继续观察真实 installed runtime；后续若扩展，只能选择 PostToolUse 最终态修复类场景，例如 VERIFIED/frontmatter 不一致。禁止把 PreToolUse gate 改成 continue block |
...
| P3 | Claude 任务列表事件兼容说明 | ✅ 已收敛。官方 hook 文档当前没有 `TodoWrite` matcher；`TaskCreated` / `TaskCompleted` 是独立事件且无 matcher，不适合作为 PACE 执行同步提醒入口 | `hooks.json` 已移除 `TodoWrite|TaskCreate|TaskUpdate` PreToolUse matcher；`task-list-sync.js` 仅保留 legacy no-op observer，避免旧手动配置报错。PACE 任务权威仍是 `changes/<id>.md ## 任务清单` |
| P3 | Release UX projected context cost | ✅ 2026-05-16 `claude plugin details paceflow@paceaitian-paceflow` 显示 Always-on 约 `296 tok`，4 个 skill on-invoke 成本合理，8 类 hook 标为 harness-only/no model context cost | `plugin details` 显示 `Agents (0)`，但 installed cache 含 `agents/artifact-writer.md` 且 manifest 声明该 agent；runtime 派 `paceflow:artifact-writer` 成功。记录为 Claude Code inventory display gap，非 release blocker；若后续 runtime dispatch 失败再升级 |
| P3 | 非锁 runtime flag 并发限制 | `.pace/stop-block-count`、legacy `task-list-used`、`degraded` 等运行态 flag 无原子写保护 | 记录为已知限制：多实例并发同项目时只可能影响提示/降级时序，不影响 artifact 内容正确性；除非真实日志显示问题，否则不加锁 |
| P3 | Deferred Stop 提醒句末双标点 | ✅ 已修复：v6.0.55 Smoke5/6 实测 Stop `systemMessage` 可见且语义正确；后续去掉 `deferredNextAction()` 内部句末标点，避免拼接成 `。。` | 纯文案 polish，已由 e2e 覆盖 deferred soft reminder |
...
- worktree 普通文件路径：当 cwd 是 git worktree，且 Write/Edit/MultiEdit 目标是宿主 checkout 中的非 artifact 普通文件时，PreToolUse 不 hard block，只输出 `additionalContext` 提醒 `artifact_dir` 仅用于 PaceFlow artifacts。若用户要求修改当前 worktree，应改用当前 worktree 下的对应路径；若用户明确要求修改宿主 checkout，可继续使用该路径。
...
- 保留边界：普通文件写宿主目录或项目外目录不由 PaceFlow 全局拦截，避免 PaceFlow 变成通用路径裁判；当前只保护 artifact / `.pace` 运行态路径，宿主 checkout 普通文件只做 artifact_dir 语义提醒。
...
- SessionStart 不把 blocked CHG 计入当前执行中的 T-NNN；SessionStart 单独展示“暂停/阻塞 CHG”，提示恢复前确认用户意图。TaskSync 已退役为 legacy observer，不再注入任务面板同步提示。
...
Smoke5/6 运行记录：ready/backlog deferred 均已验证 Stop 只输出可见 `systemMessage`、不 hard block、不增加 `.pace/stop-block-count`，且 `[ ] planned` / `planned + APPROVED` 仍不能放行项目文件写入。P3 文案问题：提醒末尾出现双句号 `。。`，语义无损；已通过去掉 `deferredNextAction()` 内部句末标点修复。

Smoke7 运行发现（2026-05-13）：blocked deferred 的 artifact 最终正确（索引 `[!]`、详情 T-001 `[!]`、工作记录有“用户要求稍后继续”、README 未写），但第一次 Stop 仍 hard block 了一次。日志显示 Stop 已先识别 `SOFT_DEFERRED_CHANGE category=blocked`，随后又因主回复含“全部完成”命中 `COMPLETION_PHRASES`，且 `stop.js` 把 deferred pending 累入 `completionPending`，触发“AI 声称完成，但 v6 详情文件中仍有 1 个未完成任务”。第二次回复不再含完成声明后才走 `SOFT_DEFERRED_PASS`。已修复：deferred CHG（backlog/ready/blocked）不参与“完成声明 + 未完成任务” hard block 计数；仍通过 Stop `systemMessage` 做人可见提醒。补 e2e：`[!]` blocked + lastMessage 含“全部完成”必须 exit 0 + systemMessage，不增加 `.pace/stop-block-count`。同时记录一个边界：主 session 把 `create-chg + approve-and-start + pause` 合成单个 artifact-writer prompt 时，PreToolUse 只看到 `operation: create-chg`，`.pace/change-owners` 可能停留在 `state=backlog`；当前 Stop 按 artifact 分类判断不受影响，但 owner runtime 状态滞后后续需观察或通过 SubagentStop/PostToolUse 复算修正。

Smoke7 后半段运行发现（2026-05-13）：同 cwd 新 session 可识别 blocked deferred 为 `owner=current-worktree`；直接写 `README.md` 先被模型语义拒绝，再被 `PreToolUse:Edit DENY_V6_E_PHASE` 机械拦截；派 `update-chg action=update-status target: CHG-20260513-01 task-id: T-001 new-status=[/]` 后写入放行；`close-chg` 后 owner 进入 `state=closed`，Stop PASS。缺口：
- 主 session 仍未调用 `Skill(paceflow:pace-workflow)` / `Skill(paceflow:artifact-management)`，仅凭 SessionStart 文案和 hook deny 重试推进。这是已记录“明确 PaceFlow 信号但模型不主动调 skill”的延续。
- `DENY_AGENT_ARTIFACT_DIR` 与 `DENY_AGENT_TARGET_REQUIRED` 两条 Agent prompt gate 只告诉缺 `artifact_dir` / `target`，没有显式带 `FORMAT_SNIPPETS.skillRef`。后续应在这两条 deny 文案末尾追加“先调用 Skill(paceflow:pace-workflow)；artifact/CHG 字段格式参考 Skill(paceflow:artifact-management)”，避免模型只机械补字段但仍不读 skill。
- 备注：本轮实际写入 `blocked write attempt` 是测试指令本身从原 smoke 文案 `blocked resume gate ok` 改成了另一行，不算 hook 缺陷；hook 不负责检查普通项目文件内容是否符合 CHG scope，只负责状态/流程 gate。close-chg 后 artifact 与实际写入内容一致，作为本次临时指令偏差记录即可，不列为待修 bug。

Block/deny skill 文案审计记录（Smoke8 后处理）：

- 已覆盖：SessionStart 对明确 PaceFlow 信号已有短入口 `先调用 Skill(paceflow:pace-workflow)`；涉及 CHG/artifact 字段再调用 `Skill(paceflow:artifact-management)`。`DENY_AGENT_LIFECYCLE_PROMPT`、reserved-id fallback/mismatch、C/E phase gate、native plan bridge、direct artifact mutation 等路径已有 skill 指向。
- 已修复缺口：`DENY_AGENT_ARTIFACT_DIR` 与 `DENY_AGENT_TARGET_REQUIRED` 是恢复型 Agent prompt block，原先只提示补字段，缺完整 `FORMAT_SNIPPETS.skillRef`；这是 Smoke7 后半段两次重试未读 skill 的直接放大点。
- 待评估：Stop hard block 当前只在第二次及以后追加 `FORMAT_SNIPPETS.skillRef`。若目标是“每次 hard block 都明确告诉模型读 skill”，可改为首次 Stop block 也附短 skillRef；但需评估是否增加 Stop 文案噪声。
- 暂不建议加：artifact runtime/lock、worktree host path protection、bad stdin/bad tool、helper CLI unknown option、resource lock busy 等安全/并发/低层 CLI block 已给出具体纠正动作，统一追加 skillRef 收益低且会增加噪声。

Smoke8 运行发现（2026-05-13）：同 cwd 新 session 对 ready deferred 的识别与恢复主目标达成。第二 session 启动后用户只输入 `hi`，模型回复中能看到当前项目有 `CHG-20260513-01 planned` 且随时可以开始；Stop `systemMessage` 可见，owner 判定为 `current-worktree`，未 hard block。用户输入“开始吧”后，模型恢复该 CHG、执行 README 修改、验证并 close/archive，最终 `task.md` / `implementation_plan.md` 活跃区为空、ARCHIVE 下 `[x]` 索引完整，详情 `status: archived` 且 `APPROVED` / `VERIFIED` 完整，owner `state=closed`，Stop PASS。
...
Claude Code `2.1.139` hook 文档新增项：`args: string[]` exec form 已完成代码迁移，用 `command: "node"` 与 `args: ["${CLAUDE_PLUGIN_ROOT}/hooks/xxx.js"]` 直接 spawn，避免 Windows / 空格路径 / 引号拼接问题；2.1.139+ installed plugin 环境已由 Smoke10/11 验证。`continueOnBlock` 的 prompt/agent hook 配置糖不直接适用于 PaceFlow command hook，但 command hook 可返回 `decision:"block" + continue:true` 达到同一 turn 修复效果；v6.0.57 仅用于 walkthrough 终态修复。

#### 0.1.10e24 Prompt Surface 统一优化计划（2026-05-14）

Anthropic 官方 `Prompting best practices` 对 Claude Opus 4.7 的核心提示是：模型更字面执行、更少自动泛化，正向示例通常比单独的负向禁令更有效。PaceFlow 的 `skills`、hook 提示、`agent.md`、`agent-references`、`CLAUDE.md` 都是 prompt surface，应统一按同一套提示工程规范维护，而不是分别堆规则。

目标不是“文案更好看”，而是减少真实生产链路中的重复失败：明确 PACEflow 信号但不调用 skill、`artifact-writer` 因缺字段连续重试、hook/skill/agent/CLAUDE.md 之间规则重复或互相稀释。

**Phase 0：基线盘点**

范围：`CLAUDE.md`、`plugin/skills/**/SKILL.md`、`plugin/agents/artifact-writer.md`、`plugin/agent-references/**`、`plugin/hooks/**/*.{js,json}` 中所有 user-facing / model-facing 文案，以及 `docs/production-smoke-*.md` 的失败信号和测试 prompt。

产出：prompt surface inventory，按 `SessionStart` / `PreToolUse deny` / `Stop block` / `Stop systemMessage` / `Skill` / `Agent` / `Reference` / `CLAUDE.md` 分类；同时记录 Smoke7/8 暴露的重试路径作为改造前基线。

**Phase 1：统一 Prompt Surface 规范**

| 官方原则 | PaceFlow 规范 |
|---|---|
| Clear/direct instructions | 每段文案必须说明适用对象、触发条件、目标动作和边界 |
| Literal instruction following | 不依赖模型从“涉及 PACEflow”推断出调用 skill 或补字段；必须显式写下一步 |
| Positive examples | 先给正确 prompt 模板，再写禁止项；避免裸写“不要自行推断” |
| Output constraints | artifact-writer 的输入/输出字段用结构化模板，不靠散文描述 |
| Tool use triggering | 有 PACE 信号时明确写“先调用 Skill(paceflow:pace-workflow)”与“artifact 字段参考 Skill(paceflow:artifact-management)” |
| Context management | `CLAUDE.md` 放长期事实；skill 放流程；reference 放细节；hook 放最小纠偏 |
| Reduce over-constraint | 大写强约束只保留在硬安全边界；普通流程用简短正向指令 |
| Verification | 每个 prompt surface 改动必须有 e2e、agent fixture 或 production smoke 验收 |

**Phase 2：Hook 文案重构**

1. `SessionStart`：legacy / artifact / root-choice / deferred 等明确 PACEflow 信号统一追加短入口：

   ```text
   处理 CHG / artifact / approve / resume / close 前，先调用 Skill(paceflow:pace-workflow)。
   涉及 artifact 字段格式时，再调用 Skill(paceflow:artifact-management)。
   ```

2. `PreToolUse:Agent` deny：`DENY_AGENT_ARTIFACT_DIR`、`DENY_AGENT_TARGET_REQUIRED`、`DENY_AGENT_LIFECYCLE_PROMPT`、reserved-id fallback/mismatch、approve/approve-and-start/close 缺字段，都改为 operation 级完整模板。原则是一次列出该 operation 的最小必填字段，不逐字段挤牙膏。

3. `Stop`：hard block 给明确下一步；deferred 继续用 `systemMessage` 允许 Stop 且人可见；首次 hard block 是否追加短 skillRef 纳入本轮评估，标准是减少无 skill 重试但不增加低层安全/并发噪声。

4. `PostToolUseFailure`：继续强调“失败不等于完成”，但恢复路径写成正向动作：读失败输出、修复、重跑验证、再 close。

**Phase 3：Skill 重构**

1. `pace-workflow/SKILL.md`：frontmatter `description` 缩短成强触发，例如“Use for PACEflow-managed coding work: create, approve, resume, verify, close, or archive CHG/HOTFIX before project file edits.” 详细触发条件留正文。
2. `artifact-management/SKILL.md`：增加 operation 模板索引，让每个 operation 一屏内能看到最小必填字段。
3. `pace-bridge/SKILL.md`：bridge / sync-plan 全部改为正向 helper 模板，不解释内部 hook 实现。
4. `pace-knowledge/SKILL.md`：减少 native tool 内部细节，改成“优先可用搜索工具；不可用时用只读 Bash `rg/find` fallback”。
5. helper 相关文案统一为：正确做法是使用 hook 提供的完整命令；无完整命令时用当前 skill 根目录推导同版本 helper；禁止扫描 `~/.claude/plugins/cache` 猜版本。

**Phase 4：artifact-writer agent prompt**

1. 顶部角色压缩为：仅做 artifact CRUD，不做技术决策；失败时报告缺字段，不自行补字段。
2. 输入契约前置：6 类 operation 的必填字段表和每类一个完整示例。
3. 输出契约保留强约束：第一行必须是 `## artifact-writer 报告`；减少重复同义禁令。
4. 新增或分散示例：`create-chg`、`approve`、`approve-and-start`、`resume [!] -> [/]`、`pause [!]`、`close-chg`、`archive-chg`、`record-correction`。

**Phase 5：CLAUDE.md 瘦身**

保留每 session 都必须知道的事实：中文输出、启用 PACEflow 时读 skill、artifact-writer 是唯一 artifact CRUD 路径、验证后 close。下沉到 skill 的内容：CHG 生命周期细节、helper 使用细节、correction 双写细节、worktree owner 细节。目标是减少 `CLAUDE.md` 与 skill 对同一流程的重复定义。

**Phase 6：Reference 结构化**

`agent-references/instructions/*.md` 统一结构：`When To Use`、`Required Input`、`Correct Prompt Example`、`Steps`、`Failure Modes`。`artifact-writer-spec.md` 保留 schema 与 invariant，不放流程教学。skill 只说明何时读取哪个 reference。

**Phase 7：内部审计 prompt 优化**

internal audit skill 明确 discovery 阶段不筛选低严重度：报告所有发现，severity 单独分级；Phase 1 不预先排除方向，Phase 2 再去重、降级、证据验证。补充 stall 防御：大文件审计先 `git log --stat` 定位 commit，再逐个 `git show`，避免大区间 diff 卡住。

**Phase 8：长任务 / 多 session 状态提示**

SessionStart 和 skill 对 backlog / ready / blocked deferred 的语义保持一致：deferred 不是完成；同 worktree 新 session 可接续；不同 worktree 默认 foreign；写入 gate 不因 deferred 放宽，必须进入 `[/]` running 才能写项目文件。

**Phase 9：反过度工程与临时文件纪律**

skill / agent 文案增加正向限制：不为一次性 artifact 操作创建临时脚本；不为通过测试硬编码路径或状态；helper 只在能稳定减少复杂度时存在，例如 `reserve-artifact-id.js`、`set-artifact-root.js`、`sync-plan.js`。

**Phase 10：明确不适用项**

本轮不引入：frontend design defaults、computer use / vision、LaTeX output、模型自我知识硬编码、prefilled responses migration、temperature/sampling 参数控制。理由：这些不属于 Claude Code plugin prompt surface 或无可控运行面。
...
可选本地手动安装检查：`test -f tests/test-install.js && node tests/test-install.js`。

Focused smoke：

1. 有 PACE 信号但未调用 skill：SessionStart / deny 必须明确提示 skill。
2. `approve-and-start` 缺字段：一次 deny 给完整模板。
3. `close-chg` 缺字段：一次 deny 给完整模板。
4. blocked deferred 新 session resume：不靠多次 agent 失败推进。
5. root-choice local/vault：helper 路径不搜索 cache。
6. artifact-writer 报告标题：不因新模板破坏首行协议。

验收标准：

- Smoke7/8 类型链路中，单个 artifact-writer 操作最多一次 hook retry。
- 明确 PACE 信号场景下，模型更稳定先调用 `pace-workflow` / `artifact-management`。
- hook deny 不再只提示单字段，而是可复制完整模板。
- `CLAUDE.md` 更短，skill/reference 分层更清楚。
- 旧保护不退化：artifact 直接写保护、C/E gate、deferred Stop、worktree owner、reserved-id、resource lock 全绿。

本轮落地状态：

- ✅ Hook runtime：`hooks.json` 已迁移为 `command:"node" + args:[...]` exec form；安装器与手动 excerpt 已支持同形态配置。真实 installed runtime 仍需 Claude Code `2.1.139+` smoke。
- ✅ Hook prompt：`DENY_AGENT_ARTIFACT_DIR` / `DENY_AGENT_TARGET_REQUIRED` / lifecycle 缺字段 deny 改为完整 operation 模板；首次 Stop hard block 追加 skill 入口。
...
审计报告：`docs/audits/audit-2026-05-14-eca8c7c-84b7d7a.md`。后续修复 commit：`601f733 fix audit followups for hooks and skills`。

对账结论：

| 类别 | 状态 | 说明 |
|---|---|---|
| C-1 / H-1~H-5 | 已修 | Windows Bash 路径规范化、native plan normalize、lifecycle 表格/模板、change-id 测试与导出均已补齐 |
| W-1~W-11 | 已修或已对齐 | record-correction helper、spec 分类、helper 路径说明、find/heredoc/.sh guard、Stop warning 分类、baseline、fixtures、README 版本占位已处理 |
| I-1 / I-2 / I-3 / I-6 / I-7 / I-9 | 已修 | helper env conflict/`--cwd`/previous-choice、stale finding 读取上限、action fallback 收紧、子模块 require 测试已补 |
| I-4 helper 失败 stdout/stderr | 设计保留 | helper 主要由 Claude Bash 调用，失败信息保留 stdout 更利于模型读取和重试；若未来统一 stderr，需同步 smoke 验证 TUI/模型可见性 |
| I-5 `require.cache delete` | 审计结论修正 | 不是死代码。env scrub/测试重载需要清理子模块缓存，否则 `constants.js` 等 env 捕获会污染结果 |
| I-8 vault worktree 普通宿主文件写保护 | 边界修正 | PaceFlow 只硬保护 artifact / `.pace` 运行态；不负责阻止 worktree 修改宿主 checkout 普通文件。普通文件越界属于用户/权限/仓库策略，不是 PaceFlow artifact 语义 |
| I-10 双标点 | 非当前缺陷 | 只剩 smoke failure-signal 示例和历史记录，不是运行时文案缺陷 |
| I-11 action-plan 历史 RFC 混排 | 本次整理 | `## 13~17` 历史 v6.0.0 架构 RFC 已移至 `docs/archive-v6-design-rfc.md`，主 action-plan 保留当前执行视图和近期决策 |

后续文档纪律：

- 新审计报告放 `docs/audits/` 或 `internal/audit-tickets/`，不要继续堆在仓库根目录。
- action-plan 顶部 `0.1 当前执行视图` 是当前优先级权威；后半历史章节只保留索引或链接。
- 审计报告保持原始证据，不回写改动；修复状态记录在 action-plan 对账段，避免审计输入和处理结论混写。

#### 0.1.10e26 v6.0.55 Smoke10/11 prompt gate 与 malformed index 收口（2026-05-15）

相关提交：

- `730f4c8 fix audit followup prompt gates`：补齐 prompt gate / skillRef / lifecycle shorthand 等审计后续。
- `6260666 fix malformed change index handling`：修复 `create-chg` 索引行粘连到注释后的 malformed index 缺口，并加入 hook 解析与阻断。

Smoke10（Prompt template gate，项目 `/mnt/k/AI/paceflow-smoke-655-prompt-template`，Claude Code `2.1.141`）验证结果：

- `SessionStart` 在 installed runtime 中正常触发，使用 `hooks[].args` exec form 后 `${CLAUDE_PLUGIN_ROOT}` placeholder 展开正常，stdout 注入包含 skill 入口、`set-artifact-root.js` 与 `reserve-artifact-id.js` helper 命令。
- `create-chg` 后 `task.md` 与 `implementation_plan.md` 的索引行均为独立行首 `- [ ] [[chg-20260515-01]] ...`，没有再粘到说明注释或 `<!-- ARCHIVE -->` 前文本；详情文件保持 `status: planned`，README 未被提前修改。
- 故意派 `approve-and-start` 且省略 `task-id`：`PreToolUse:Agent` 在 Agent 启动前返回 `DENY_AGENT_LIFECYCLE_PROMPT`，错误信息明确缺 `task-id`，并给出完整 `operation: update-chg` / `action: approve-and-start` 模板与 skill 引导；没有创建额外长跑 subagent。
- 故意派 `close-chg` 且省略 `verification-confirmed` / `verify-summary` / `walkthrough-summary`：同样在 Agent 启动前被 `DENY_AGENT_LIFECYCLE_PROMPT` 拒绝，返回完整 `close-chg` 模板和“主 session 已运行并读取验证结果”的语义约束；没有创建额外长跑 subagent。
- Stop 对未批准 planned CHG 输出 `systemMessage`：`PACEflow: 仍有 deferred CHG 可后续处理...`，允许结束且人可见，不再静默 PASS。
- 随后 happy path 复测通过：`approve-and-start` 一次成功，README 第 3 行写入 `prompt template ok`，`close-chg` 一次归档完成。终态为 `changes/chg-20260515-01.md status: archived`，T-001 `[x]`，`APPROVED` / `VERIFIED` 完整，`task.md` / `implementation_plan.md` 索引位于 ARCHIVE 下方，`walkthrough.md` 有完成记录，`.pace/change-owners/chg-20260515-01.json state=closed`，Stop PASS。

Smoke11（record-finding artifact_dir 模板）验证结果：

- 故意派 `artifact-writer record-finding` 且不在 prompt 顶部写 `artifact_dir`，`PreToolUse:Agent` 返回 `DENY_AGENT_ARTIFACT_DIR`，包含正确 artifact_dir、完整 `operation: record-finding` 模板、skill 引导与 artifact_dir 范围说明。
- 该测试证明 record-finding 这类非 CHG lifecycle operation 也已纳入完整模板 gate，不再只提示单字段。

残留观察：

- `artifact-writer` production 报告仍偶发在 `## artifact-writer 报告` 前输出一句自然语言，`SubagentStop` 记录 `WARN issue=title-prefix`。功能链路、artifact 终态、Stop gate 均正常；继续按既有决策作为 P3/非 blocker 观察，不升为 release blocker。
- Smoke10/11 已覆盖 `hooks[].args` exec form 在 Claude Code `2.1.141` installed runtime 下的核心语义；Smoke15 已验证 command-hook `continue:true` PoC，v6.0.57 已把该能力限定迁移到 walkthrough 终态修复。

#### 0.1.10e27 2026-05-15 audit 6260666 收口决策

审计输入：`docs/audits/audit-2026-05-15-6260666.md`。

处理结论：

- `verify.js` 三连问题只影响手动安装/本地健康检查，不影响 marketplace runtime；该文件被 `.gitignore` 排除，不属于本次提交面。当前本地副本已核对 `hooks[].args` exec-form、拆分后的 `PACE_VERSION` 读取和配置一致性脚本名提取；后续若恢复 tracked 手动安装工具，再单独纳入提交。
- Artifact resource lock read path 缺 TTL 自愈属实，按运行时问题修复：读取锁时若发现 stale / malformed lock，自动清理并继续正常 gate。
- `CLAUDE.md` prompt surface 问题属实：仓库内外两个 `CLAUDE.md` 不再承载个人回复风格或 PACEflow workflow 规则。当前文件只作为维护入口；运行规则归位到 hooks、skills、agent references、README/REFERENCE。
- Skill/reference 文档收口：补 `update-status` 必填 `section=tasks/new-status`、legacy v5 迁移与 worktree 共享语义、现有 references 链接、finding type 枚举一致性与 Obsidian skill 命名。
- 历史文档收口：v5 hook messages、v6 guidebook、agent design、v5 archival strategy、CHANGELOG 均加 historical 说明；README 明确 v6 历史以 README 表为准，CHANGELOG 只保留 v5 快照。

验证要求：

- 必跑：`node tests/test-pace-utils.js`、`node tests/test-hooks-e2e.js`、`node --check` 关键 hook 文件、`git diff --check`、`claude plugin validate ./plugin`。
- 本轮已跑：`node tests/test-pace-utils.js` 139/139 PASS；`node tests/test-hooks-e2e.js` 216/216 PASS；`node --check` 关键 hook 文件 PASS；`git diff --check` PASS；`claude plugin validate ./plugin` PASS。`node tests/test-install.js` 26/26 PASS 属本地 ignored manual-install tooling 补充检查。
...
- 必跑：`node tests/test-pace-utils.js`、`node tests/test-hooks-e2e.js`、关键 hook `node --check`、`git diff --check`、`claude plugin validate ./plugin`。
- 本轮已跑：`node tests/test-pace-utils.js` 139/139 PASS；`node tests/test-hooks-e2e.js` 216/216 PASS；关键 hook `node --check` PASS；`git diff --check` PASS；`claude plugin validate ./plugin` PASS。`node tests/test-install.js` 26/26 PASS 属本地 ignored manual-install tooling 补充检查。


#### 0.1.10e29 Claude Code 2.1.139+ hook/tool 能力复核与执行方案（2026-05-16）

输入：Claude Code 官方 changelog / hooks reference / tools reference / plugins reference，复核范围 `2.1.139` 至 `2.1.143`。

已确认事实：

- `hooks[].args` exec form 是 `2.1.139` 新增能力，当前 PaceFlow 已迁移为 `command:"node" + args:["${CLAUDE_PLUGIN_ROOT}/hooks/*.js"]`，Smoke10/11 在 2.1.141 installed runtime 下确认 placeholder 展开、stdin/stdout/exit 语义正常。`2.1.138` 及更早版本不支持该字段，继续要求用户使用 Claude Code `2.1.139+`。
- `PostToolUse continue:true` / `continueOnBlock` 类能力的价值不是替代 PreToolUse，而是在工具已经执行后，把“终态必须修复”的 PostToolUse 拒绝原因喂回 Claude 并继续当前 turn，减少 close/archive、walkthrough、finding/correction 这类机械修复的重派成本。PACEflow 生产实现使用 command hook 返回 `decision:"block" + continue:true`。
- `terminalSequence` 是 `2.1.141` 新增能力，可用于通知/铃声/窗口标题，不适合作为 PaceFlow 状态权威；Stop `systemMessage` 仍是 deferred/soft reminder 的主通道。
- `PowerShell` 是独立工具名，不是 Bash 的别名；2.1.143 扩大 Windows 默认启用面且 PowerShell 默认带 `-ExecutionPolicy Bypass`。当前 PaceFlow 只监听 `Bash`，因此 PowerShell 写 artifact 或 `.pace` runtime-control 是新的 P1 工具面缺口。
- `Monitor` 可运行后台命令，官方说明它复用 Bash permission rules；需要实测 hook matcher 是否以 `Monitor` 独立工具名触发，避免后台命令绕过 artifact/runtime 写保护。
- `worktree.bgIsolation:"none"` 会让 background session 直接编辑同一 working copy，不符合 PaceFlow 当前“每个 worktree 单 owner / artifact owner 归属”假设；不作为推荐配置。
- `claude plugin details` / `/plugin` projected context cost 可用于 release UX 检查，帮助控制 skills/agents/hooks token 成本；不影响运行时正确性。
- 2.1.143 的 Stop 连续 block cap 与 PaceFlow 自身三次降级不冲突；PaceFlow 仍保留自己的 3 次降级和 deferred soft reminder 语义。

执行方案：

1. P1：补 PowerShell/Monitor 工具面保护（v6.0.56 已落地）。
   - 注册面：`plugin/hooks/hooks.json` 的 `PreToolUse` 已增加 `PowerShell` 与 `Monitor`；`PostToolUseFailure` 也同步覆盖两者。
   - 代码面：新增 `plugin/hooks/pre-tool-use/powershell-guard.js`，PowerShell 语法不混入 Bash guard。覆盖写语义：`Set-Content` / `Add-Content` / `Out-File` / `New-Item` / `Remove-Item` / `Move-Item` / `Copy-Item`、`>` / `>>`、常见 alias（`sc` / `ac` / `ni` / `rm` / `del` / `mv` / `cp`）、`node -e` / `python -c` / `.ps1` wrapper 中的 artifact/runtime 写入。
   - 边界：继续只保护 PaceFlow artifacts 和 `.pace` runtime-control，不扩大成“项目外/宿主 checkout 普通文件”通用路径裁判。
   - 测试：E2E 模拟 `tool_name:"PowerShell"`；覆盖 `task.md`、`changes/**`、`.pace/locks/**` 写入 deny，`Get-Content` / 只读命令 PASS，Windows 路径和相对路径等价；worktree 本地 `.pace/artifact-root` 写入继续提示改用 helper。Monitor 覆盖最小 deny/pass fixture。
   - Runtime smoke：在 Windows Claude Code 启用 PowerShell tool 后复测 artifact 写保护；确认 PowerShell deny 文案仍引导 Skill 和 artifact-writer，不建议模型改用 Bash 绕过。

2. P2：做 `PostToolUse decision:block + continue:true` 隔离 PoC（2026-05-16 已通过）。
   - PoC 目标：不改现有主 hook，新增最小测试 hook 或 fixture，确认 command-type `PostToolUse` 是否能通过 `decision:"block"` + `continue:true` 在同一 turn 继续，让 Claude 看到 rejection reason 后修正。
   - 结果：`/mnt/k/AI/cc-smoke15` 中首次 `Write(poc.txt)` 后，hook 返回 block reason，Claude 同一 turn 追加 `repair line`，第二次 PostToolUse 因 one-shot marker 放行。
   - 进入条件：只在 PoC 证明 command hook 可用且不会制造循环时，才迁移真实场景；该条件已满足，但仍需要单独设计 production 试点。
   - 候选迁移场景：walkthrough wikilink/context 缺失、verified marker/frontmatter 终态不一致、record-finding / record-correction 双写不完整、close/archive 后索引/详情最终态不一致。
   - 禁止迁移场景：artifact-root 选择、legacy v5 migration、C/E gate、agent missing required fields、直接 artifact mutation、Bash/PowerShell runtime-control 写入。这些仍必须在 PreToolUse 阶段 deny。
   - 防循环：每 target/session 只 continue 一次或用 `.pace` per-session flag 限流；超过限流降级为 additionalContext/Stop。

3. P3：采用低风险 Claude Code 新能力作为 release/UX 检查。
   - `claude plugin details` / `/plugin` projected context cost：已完成一次检查。v6.0.56 always-on 约 `296 tok`，hooks 为 harness-only/no model context cost；`Agents (0)` 与 installed manifest/cache/runtime dispatch 不一致，按 Claude Code inventory display gap 记录。
   - `terminalSequence`：只作为可选本地通知实验，不进入默认 runtime；不得替代 Stop `systemMessage`。
   - `claude agents` 新 flags：可用于 production smoke 启动不同 `--plugin-dir` / `--settings` / `--model` / `--effort` 组合，不作为 PaceFlow 用户流程依赖。

验收顺序：

1. PowerShell/Monitor coverage 已进入 v6.0.56；提交前必须跑 `node tests/test-hooks-e2e.js`、`node tests/test-pace-utils.js`、`git diff --check`、`claude plugin validate ./plugin`，发布后跑 Windows installed Smoke14。`node tests/test-install.js` 只在本地 ignored manual-install tooling 存在时作为补充检查。
2. `decision:block + continue:true` PoC 已通过；v6.0.57 已实现最小 production 试点：walkthrough wikilink/context 终态修复 + one-shot 限流。
3. projected context cost 已记录；`terminalSequence` / optional notification 仍不进入默认核心链路。
...
- 回归验证：新增 `9haa0` / `9haa0b` 覆盖无 PACE 信号的显式 artifact-writer 入口；当前基线为 `node tests/test-hooks-e2e.js` 237/237 PASS，`node tests/test-pace-utils.js` 159/159 PASS；`node tests/test-install.js` 26/26 PASS 为本地 ignored manual-install tooling 补充检查。


#### 0.1.10e30 旧 Superpowers plan 与 TaskSync 提醒边界（2026-05-16）

Windows `paceflow` installed runtime 观察：

- `docs/plans/2026-03-08-coverage-enhancement.md` 与同名 `-design.md` 是较早前遗留的计划文件，但当前 `formatBridgeHint()` 只看日期命名 plan 是否未写入 `.pace/synced-plans`，不判断 plan 是否仍是当前工作。
- `TaskUpdate` 触发 `task-list-sync.js` 后，hook 把“Superpowers 计划文件存在但 task.md 不存在，写代码前先 bridge”的提醒作为 `additionalContext` 注入。Claude 能读到，但用户在 TUI 不可见；如果当前没有写代码、没有派 artifact-writer，也没有触发任何 PACE hard gate，这类提醒会重复进入模型上下文。

结论：

1. 旧 plan 不能无限期作为当前 bridge 信号。
   - `.pace/synced-plans` 只能表示“已 bridge”，不能表示“此 plan 仍应参与当前执行”。
   - Bridge hint 应区分当前/近期/显式选中的 plan 与历史 plan。历史 plan 可留在 `docs/plans/`，但不应长期污染 SessionStart、TaskSync 或普通工作上下文。

2. TaskSync 的语义边界需要收紧。
   - `TaskCreate` / `TaskUpdate` / `TodoWrite` 是 Claude 任务列表工作记忆，不是 PaceFlow artifact 权威。
   - 在 `superpowers + no task.md` 场景下，TaskSync 不应持续通过用户不可见的 `additionalContext` 注入 bridge 提醒。
   - 真正需要强提醒的时机是 `Write/Edit/MultiEdit/Bash/PowerShell/Monitor/Agent` 准备写代码或进入 artifact 流程时，由 PreToolUse hard gate 给出用户可观察的阻断与 bridge 指令。

候选修复方案：

- `formatBridgeHint()` 增加候选过滤，只返回：
  - `.pace/current-native-plan` 指向且仍匹配当前项目的 plan；
  - 最近修改或创建的 plan（阈值待定，例如 7~14 天）；
  - 用户通过显式 helper/运行态标记选择的 plan。
- 对超出 freshness 窗口的 plan，降级为历史 backlog：可在日志中记录，不触发 TaskSync `additionalContext`；SessionStart 最多给一次短提示或完全静默，避免旧计划变成永久启动噪声。
- `task-list-sync.js` 对 `superpowers + no task.md` 改为 log-only，或一会话限流一次轻量 `additionalContext`。默认倾向 log-only，因为真正的桥接要求会在 PreToolUse 写入路径上硬阻断。
- 保持既有 hard gate：代码/非代码项目写入、artifact-writer 派遣、Bash/PowerShell/Monitor artifact 写入仍必须先 bridge 或 create CHG，不因 TaskSync 降噪而放松。

验收：

- 存在 30 天前 `docs/plans/*.md` 且没有 `.pace/synced-plans` 时，`TaskUpdate` 不再反复注入 bridge 提醒。
- 新近 plan 或显式 current-native-plan 仍会在真正写代码/派 artifact-writer 前被 PreToolUse 拦住并要求 `paceflow:pace-bridge`。
- e2e 覆盖旧 plan log-only、新 plan bridge hard gate、TaskSync 不污染上下文三类路径。
...
- `task-list-sync.js` 对 non-artifact 信号曾先改为 log-only；随后按当前官方 hook 文档继续收敛：插件不再注册任务面板 hook，避免任何用户不可见 `additionalContext` 进入模型上下文。
- 回归：`tests/test-pace-utils.js` 覆盖旧 plan / current-native-plan；`tests/test-hooks-e2e.js` 覆盖 Superpowers no-task 与 code-count no-artifact TaskSync 静默及 legacy observer no-op。

#### 0.1.10e30b artifact 模式下 TaskSync 与 Claude 任务面板边界（2026-05-23）

Smoke17G 观察：

- 一个很小的 CHG 在已派 `artifact-writer` 的情况下，Claude 主 session 使用 `TaskCreate` / `TaskUpdate` 管理自己的任务面板，触发了多次 `TaskSync additionalContext`。
- 典型重复提示包括“当前无活跃 CHG，先 create-chg”与“当前执行中的 CHG 有 N 个未完成 T-NNN”。这些提示进入模型上下文，但用户在 TUI 不一定可见，且 close 后清理任务面板时仍可能出现“先 create-chg”的反向噪声。
- hook 日志本身可读且用于诊断，不需要降噪；真正需要收敛的是用户不可见的 `additionalContext` 注入范围。

边界结论：

- `TaskCreate` / `TaskUpdate` 是 Claude 任务面板工作记忆，PACEflow 不应规定主模型如何拆解 P/A/C/E/V 或 UI 任务项。
- 当前官方 hook 文档没有 `TodoWrite` matcher；任务面板相关 hook 是独立 `TaskCreated` / `TaskCompleted` 事件，且不支持 matcher。
- `TaskCreated` / `TaskCompleted` 更适合“阻止任务创建/完成”的 UI 规则，不适合作为 PACE 执行同步提醒入口；迁移过去会继续让 PaceFlow 管主模型任务面板，违背本轮边界。
- close 后的 `TaskUpdate status=completed` 属于工作记忆清理，不应提示重新 create CHG。

落地：

- `hooks.json` 移除 `PreToolUse` 的 `TodoWrite|TaskCreate|TaskUpdate` matcher。
- `task-list-sync.js` 保留为 legacy 兼容 observer：旧手动配置若仍调用它，只记录运行态并保持 stdout 为空，不再注入 `additionalContext` 或阻断。
- 回归：`tests/test-hooks-e2e.js` 覆盖 hooks.json 不再注册任务面板 matcher、legacy observer 无输出、`TaskCreated` / `TaskCompleted` 事件输入无输出。

后续文案补强方案：

- `SessionStart` 和 skills 是合适的单次提醒面：新会话、resume、clear、compact、主动调用 `paceflow:pace-workflow` / `paceflow:artifact-management` 时提醒即可。SessionStart 中这段使用“CHG 执行上下文”命名，不再使用“Claude 任务列表同步”命名。
- 不新增高频 hook 提醒，不在 `TaskCreate` / `TaskUpdate` / `TaskCreated` / `TaskCompleted` 做同步提醒，也不因“未证明已读 CHG”做 hard gate。
- 正向规则文案：继续、恢复或收口已有 CHG 前，先 Read `changes/<slug>.md`，确认任务清单、实施详情和工作记录；`SessionStart` 摘要只用于定位，不替代 CHG 详情。
- 阻断型 gate 只在已有明确语义错误时顺带提示读详情，例如 verify/close 缺字段、target/task-id 缺失、状态不一致；不要单独因为“没读详情”阻断。
- 验收：active / ready / deferred / blocked CHG 的启动摘要和 workflow / artifact-management skill 都包含这条短规则；installed smoke 观察主 session 接续 CHG 时先读详情的概率是否提升，且不出现反复不可见 context 噪声。
...
| 前次 #2 todowrite-sync matcher 死代码疑虑 | 已被 2026-05-23 官方文档复核取代：当前无 `TodoWrite` matcher；任务面板 hook 不再注册 | 2026-05-23 | 官方 hooks 文档复核 |

---

## 3. P0 立即可做（最高优先级）

### CHG-20260502-01：hooks.json 加 `if` 条件优化（已废弃）

**当前结论（2026-05-23）**：本项不再执行。官方 hook 文档当前没有 `TodoWrite` matcher；`TaskCreated` / `TaskCompleted` 是独立事件且无 matcher。PACEflow 已移除任务面板 hook 注册，`task-list-sync.js` 仅保留 legacy no-op observer。

历史目标：减少不必要的 hook 进程启动开销。
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "<retired task-panel matcher>",
        "if": "<same retired task-panel matcher>",
        "hooks": [...]
      }
    ]
  }
...
**风险**：当前不适用；继续保留该 matcher 会与官方 Hook 事件模型不一致。
...
`## 13~17` 的 v6.0.0 历史架构 RFC 已移至 [archive-v6-design-rfc.md](archive-v6-design-rfc.md)。当前执行优先级以本文顶部 `0.1 当前执行视图` 为准。
