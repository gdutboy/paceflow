...
> **生成日期**：2026-05-02
> **当前执行版本**：PACEflow v6.0.54（原始调研输入：PACEflow v5.1.4）
...
node tests/test-hooks-e2e.js                         # 163/163 PASS
...
| 优先级 | 缺口 | 当前状态 | 下一步 |
|---|---|---|---|
| P1 | Installed-plugin production smoke | ✅ v6.0.41 Smoke0-7 已完成；Smoke6 复测确认 Bash / Write / Edit 直接改流程 artifact 均被拦，`spec.md` Edit 例外正常；marketplace cache 运行时发布面干净。v6.0.51 已把 `pre-tool-use.js` helper 拆分后的 runtime packaging smoke 写入文档 | 发新版或改 hook/agent runtime 时按 `docs/production-smoke-v6.0.51.md` 复跑：helper packaging、local create+close、direct artifact write protection、worktree concurrent create、vault route、v5 migration guard、native `/plan` bridge sync |
...
| P2 | Production smoke 文档更新 | ✅ v6.0.41 已更新 `docs/production-smoke-v6.0.41.md`；v6.0.48 已新增 focused smoke；v6.0.51 已新增 `docs/production-smoke-v6.0.51.md`，覆盖 PreToolUse helper packaging、resource-lock 并发模型、reserved-id 二次派遣、direct artifact 写保护、vault/local 路由、v5 migration guard 与 native `/plan` bridge sync | 后续版本按 v6.0.51 focused smoke 复跑关键链路；只有大改生命周期语义时再扩展全量手册 |
