...
本机已复核到 `2.1.138`；2026-05-14 已将 hook 注册迁移为 `args: string[]` exec form，`claude plugin validate ./plugin` 在当前本机通过，单元/e2e 覆盖配置结构。真实 installed runtime 仍建议在 Claude Code `2.1.139+` 上做一次 smoke，确认 `args` 中 plugin placeholder 展开和 stdin/exit/stdout 语义一致。`PostToolUse continueOnBlock` 仍先保留为后续 PoC。
...
| P2-hardening | Claude Code v2.1.139 hook runtime modernization | `args: string[]` exec form 已完成代码迁移并通过 `claude plugin validate` / e2e；`PostToolUse continueOnBlock` 可能降低 PostToolUse 提醒导致的 turn 中断/重试 | 后续只保留两个验证项：1）在 Claude Code 2.1.139+ installed plugin 环境做一次 smoke，确认 plugin placeholder 在 args 中展开；2）隔离验证 command PostToolUse 是否可用 `decision:"block" + continue:true` 等价获得 continueOnBlock 体验。若只对 prompt/agent hooks 生效，则不迁移现有 command PostToolUse |
