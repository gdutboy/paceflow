...
- `PostToolUse continueOnBlock` 是 `2.1.139` 新增能力，价值不是替代 PreToolUse，而是在工具已经执行后，把“终态必须修复”的 PostToolUse 拒绝原因喂回 Claude 并继续当前 turn，减少 close/archive、walkthrough、finding/correction 这类机械修复的重派成本。
...
2. P2：做 `PostToolUse continueOnBlock` 隔离 PoC（2026-05-16 已通过）。
...
1. PowerShell/Monitor coverage 已进入 v6.0.56；提交前必须跑 `node tests/test-hooks-e2e.js`、`node tests/test-install.js`、`node tests/test-pace-utils.js`、`git diff --check`，发布后跑 Windows installed Smoke14。
2. `continueOnBlock` PoC 已通过；v6.0.57 已实现最小 production 试点：walkthrough wikilink/context 终态修复 + one-shot 限流。
