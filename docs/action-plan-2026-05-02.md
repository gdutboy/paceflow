...
| P2-hardening | Claude Code v2.1.139 hook runtime modernization | ✅ `args: string[]` exec form 已完成代码迁移并通过 `claude plugin validate` / e2e；2026-05-15 在 Claude Code 2.1.141 installed runtime 下完成 Smoke10/11，确认 plugin placeholder 在 args 中展开，hook stdin/exit/stdout 语义正常。`PostToolUse continueOnBlock` 可能降低 PostToolUse 提醒导致的 turn 中断/重试 | exec-form 已从待验证项移除；后续仅保留隔离验证 command PostToolUse 是否可用 `decision:"block" + continue:true` 等价获得 continueOnBlock 体验。若只对 prompt/agent hooks 生效，则不迁移现有 command PostToolUse |
...
| P2-POC | `PostToolUse continueOnBlock` 即时修复循环 | 待验证。适用目标是“工具已执行后发现终态结构必须修复”的场景，例如 verified/frontmatter 终态不一致、walkthrough wikilink/context 缺失、finding/correction 双写缺口；不适用于 root choice、C/E gate、artifact 直接写入等本应在 PreToolUse 拦截的场景 | 先做隔离 hook PoC，确认 command hook 能否用 `decision:"block"` + `continueOnBlock:true` 把原因喂回模型并继续本 turn。通过后再把少量最终态 PostToolUse warning 升级；不通过则保留 additionalContext/Stop 现状 |
...
2. P2：做 `PostToolUse continueOnBlock` 隔离 PoC。
   - PoC 目标：不改现有主 hook，新增最小测试 hook 或 fixture，确认 command-type `PostToolUse` 是否能通过 `decision:"block"` + `continueOnBlock:true` 在同一 turn 继续，让 Claude 看到 rejection reason 后修正。
   - 进入条件：只在 PoC 证明 command hook 可用且不会制造循环时，才迁移真实场景。
...
3. P3：采用低风险 Claude Code 新能力作为 release/UX 检查。
   - `claude plugin details` / `/plugin` projected context cost：加入 release checklist，观察 skills/agents/hooks token 成本是否异常膨胀。
...
2. 后续再做 `continueOnBlock` PoC；PoC 不通过时只记录结论，不改 production PostToolUse。
3. 最后补 release checklist 的 projected context cost / optional notification，不进入核心阻断链路。
