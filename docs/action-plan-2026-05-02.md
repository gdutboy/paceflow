...
> **上游调研版本**：Claude Code v2.1.126（后续本机复核至 v2.1.138）
...
本机已复核到 `2.1.138`；当前 smoke 暴露的问题仍来自 PaceFlow 提示/门禁边界，而不是需要改变既有 PACEflow hook/task 决策的 Claude Code 行为。
...
| 优先级 | 缺口 | 当前状态 | 下一步 |
|---|---|---|---|
| P1 | Installed-plugin production smoke | ✅ v6.0.54 Smoke1-6 已复跑并记录；worktree walkthrough context 已补齐。本轮 v6.0.55 改动集中在 helper 入口、当前 owner 非代码 C/E gate、SubagentStop owner closed 兜底与 skill 入口提示 | 发新版或改 hook/agent runtime 时按 v6.0.55 focused smoke 复跑：root-choice vault/local helper、current-owner README/文档 C/E gate、foreign fresh worktree 并发、close/archive owner closed、v5 migration guard、native `/plan` bridge sync |
...
| P2 | Production smoke 文档更新 | ✅ v6.0.41 已更新 `docs/production-smoke-v6.0.41.md`；v6.0.48/51 已新增 focused smoke；v6.0.54/55 的新增验证目标已记录在本 action book | 后续版本按 v6.0.55 focused smoke 复跑关键链路；只有大改生命周期语义时再扩展全量手册 |
