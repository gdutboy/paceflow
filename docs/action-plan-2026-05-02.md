...
本机已复核到 `2.1.141`；2026-05-14 已将 hook 注册迁移为 `args: string[]` exec form，`claude plugin validate ./plugin` 在当前本机通过，单元/e2e 覆盖配置结构。2026-05-15 在 Claude Code `2.1.141` installed runtime 下完成 Smoke10/11：SessionStart、PreToolUse、SubagentStop、Stop 均正常触发，说明 `args` 中 plugin placeholder 展开与 stdin/exit/stdout 语义在当前环境可用。`PostToolUse continueOnBlock` 仍先保留为后续 PoC。
