...
| C 阶段批准并立即开始 | `update-chg action=approve-and-start approval-confirmed:true` |
...
`<!-- APPROVED -->` 不在 `task.md`。它属于 CHG 详情文件。create 时不写，C 阶段由 `update-chg action=approve` 插入；若用户批准后立即开始，则由 `approve-and-start` 插入并同步首个任务 `[/]`。
...
agent 在详情文件任务清单后插入 `<!-- APPROVED -->`。`approve-and-start` 会同时把指定任务标 `[/]`，并联动 frontmatter `status: in-progress`、主索引 `[ ] -> [/]`；单独 `approve` 不改变 status。
