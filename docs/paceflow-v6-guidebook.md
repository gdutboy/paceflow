...
| C 阶段批准并立即开始 | `update-chg action=approve-and-start` |
| 更新任务状态 `[ ]` / `[/]` / `[x]` / `[-]` | `update-chg action=update-status` |
| V 阶段验证，写 `<!-- VERIFIED -->` + `verified-date` | `update-chg action=verify` |
| 归档 CHG | `archive-chg` |
| 验证后完成并归档 CHG | `close-chg` |
...
| `closing-required` | `[x]` 或 `status=completed` | 阻止直到 verified + archive；推荐验证通过后派 `close-chg` |
...
验证通过后，主 session 派 agent 记录验证结果到 `changes/<id>.md` 的 `## 工作记录`，并写入 v6 定义的 `<!-- VERIFIED -->` 标记。推荐在准备收尾时直接派 `close-chg verification-confirmed: true`，一次完成 verified + archive；若仍采用 v5 的 `task.md` VERIFIED，hook 会误判或把多个 CHG 混成一个验证状态。
...
- V 阶段：验证结果写入详情文件，由 agent 记录；验证通过并收尾时调用 `close-chg`。
- G-9 完成检查改成 “验证通过后 agent close-chg/archive-chg，hook 验证活跃索引清空”。
