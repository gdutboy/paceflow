...
> 执行状态更新（2026-05-06，v6.0.15）：agent 契约新增 `update-chg action=approve-and-start` 与 `close-chg`，用于合并“批准+开始”和“完成+验证+归档”。合并不替代用户确认或验证执行；主 session 必须在派遣前确认 `approval-confirmed: true` / `verification-confirmed: true`。
...
> 执行状态更新（2026-05-08，v6.0.31-v6.0.33 历史）：当时 `artifact-writer` 使用项目级写锁并修复 Agent 失败释放、Bash 删锁/伪锁竞态；真实 git worktree 与主项目共享运行态，hook 日志使用 Claude Code `session_id` 串联，`T-NNN` 明确为 CHG/HOTFIX 局部编号。
...
PACEflow v6 的核心改变不是“换一种 artifact 格式”，而是把 artifact CRUD 从主 session 移到 `artifact-writer` agent，并把主索引文件变成轻量 wikilink 索引。主 session 保留业务判断、用户交互、代码实现和 hook 修复；artifact 的创建、更新、归档、finding/correction 记录必须派 agent。

当前状态分三层：

| 层 | 当前状态 | v6 判断 |
|---|---|---|
| Agent | `agents/artifact-writer.md` v6-only；frontmatter `name: artifact-writer` + `color: orange` + `effort: max`；写入受 hook resource lock 保护 | 可作为 v6 artifact 操作权威执行器；继续用 production smoke 防回归 |
| Hook | 核心 hook 已按 `changes/` 详情 + wikilink 索引运行；legacy fallback 统一提示迁移/桥接；worktree 路由、ID reservation 与 artifact resource lock 已覆盖 | 代码层验证已过；剩余重点是 installed-plugin 真实 smoke |
...
| 历史审计输入 CLAUDE | `CLAUDE.md` | 初始审计发现它仍按 v5 artifact 权威、5 文件双区和 `findings.md` correction 运行；当前已修复为 v6-only，旧口径仅作历史问题来源 |
...
如果 cwd 位于 `worktrees/<name>` 下，`<project-name>` 取 `worktrees` 的父目录名；如果显式设置 `PACE_PROJECT_NAME` / `PACEFLOW_PROJECT_NAME`，则以显式项目名优先。
...
以下任何 artifact 操作都必须派 `artifact-writer`：

| 操作 | Agent operation |
|---|---|
| 创建 CHG/HOTFIX/research 变更 | `create-chg` |
| 更新 CHG 任务、实施、工作记录、调研段 | `update-chg` |
| C 阶段仅批准，暂不开始 | `update-chg action=approve approval-confirmed:true approval-source:<source> approval-evidence:<evidence>` |
| C 阶段批准并立即开始 | `update-chg action=approve-and-start approval-confirmed:true approval-source:<source> approval-evidence:<evidence> task-id:T-NNN` |
| 暂停、阻塞、跳过、跨 session、长任务进度可见性时更新任务状态 `[ ]` / `[/]` / `[x]` / `[-]` | `update-chg action=update-status` |
| 只记录 V 阶段暂不归档，写 `<!-- VERIFIED -->` + `verified-date` | `update-chg action=verify` |
| 归档 CHG | `archive-chg` |
| 最后任务验证后完成并归档 CHG | `close-chg complete-open-tasks:true` |
...
reserved-id: CHG-YYYYMMDD-NN
reserved-file: changes/chg-yyyymmdd-nn.md
fields:
  title: "..."
  tasks:
    - "T-001: ..."
...
| `closing-required` | `[x]` 或 `status=completed` | 阻止直到 verified + archive；推荐验证通过后派 `close-chg complete-open-tasks:true` |
...
`<!-- APPROVED -->` 不在 `task.md`。它属于 CHG 详情文件。create 时不写，C 阶段仅批准暂不开始时由 `update-chg action=approve` 插入；若用户批准后立即开始，则由 `approve-and-start` 插入并同步首个任务 `[/]`。两条路径都必须带确认字段。
...
用户批准后主 session 派 agent。若马上开始首个任务，优先合并为：
...
agent 在详情文件任务清单后插入 `<!-- APPROVED -->`。`approve-and-start` 会同时把指定任务标 `[/]`，并联动 frontmatter `status: in-progress`、主索引 `[ ] -> [/]`；单独 `approve` 不改变 status，且不得在同一 prompt 中表达开始执行。`approval-evidence` 只用于审计，hook 不判断其真伪。
...
CHG 不是大计划容器，而是连续执行、可验证、可关闭的最小变更单元。大计划应拆成多个 CHG；一个 CHG 内可以有多个 T-NNN，但这些任务应服务于同一个闭环，并默认在同一执行流里连续完成。

因此 E 阶段主路径不是“每完成一个任务就派一次 `update-status`”，而是：

1. 继续完成同一 CHG 的代码修改与必要测试。
2. 如果仍在同一轮连续执行，不为中间 T-NNN 单独派 `update-status`。
3. 只有暂停、阻塞、跳过、跨 session、长任务进度可见性，或最后任务暂不验证/暂不收尾时，才派 `update-chg action=update-status`。
4. 验证通过并准备收尾时，直接派 `close-chg complete-open-tasks:true`，由 agent 一次完成 open tasks 标记、VERIFIED、归档和 walkthrough。

### 5.5 V 阶段：Verify

验证通过后，主 session 派 agent 记录验证结果到 `changes/<id>.md` 的 `## 工作记录`，并写入 v6 定义的 `<!-- VERIFIED -->` 标记。推荐在准备收尾时直接派 `close-chg verification-confirmed: true complete-open-tasks: true`，一次完成最后任务收口、completed、verified + archive；若仍采用 v5 的 `task.md` VERIFIED，hook 会误判或把多个 CHG 混成一个验证状态。若只记录验证、暂不归档，才派 `update-chg action=verify`。

### 5.6 Archive

验证已确认通过后，主 session 优先派 agent：
...
1. 若 `complete-open-tasks:true`，把当前执行与验证覆盖的 open tasks 一次收口为 `[x]`；否则验证所有任务已是 `[x]` 或 `[-]`。
...
1. 注入 5 个索引文件活跃区，并在需要时补充 `spec.md` 项目元数据。
...
- 删除 “5 个核心 Artifact” 改为 “5 个索引文件 + spec.md + changes 详情”。
...
- C 阶段：用户批准并立即开始时调用 `update-chg action=approve-and-start`；只批准暂不开始时才调用 `update-chg action=approve`。二者都必须携带 `approval-confirmed/source/evidence`。
- E 阶段：默认连续执行同一 CHG 的代码修改与测试，不逐个 T-NNN 派 `update-status`；只有暂停、阻塞、跳过、跨 session、长任务进度可见性，或最后任务暂不验证/暂不收尾时，才调用 `update-chg action=update-status`。工作记录需要单独追加时调用 `update-chg section=work-record action=append`。
- V 阶段：主 session 先运行并读取验证结果；验证通过并收尾时调用 `close-chg complete-open-tasks:true`。
- G-9 完成检查改成 “最后任务验证通过后 agent close-chg，hook 验证活跃索引清空”；`archive-chg` 仅作为已 verified 的单独归档修复。
...
### 7.5 `internal/skills/audit`

v6 初始审计时审计范围没有 `agents/`、`changes/`、v6 tests；prompt 仍要求读 `CLAUDE.md` v5 章节。v6.0.29 已将该能力从 marketplace 发布面移到 `internal/skills/audit/`，作为 PaceFlow 仓库自身质量门控资料，不再作为用户项目 skill 安装。
...
4. 改写 corrections：用户纠正必须派 `artifact-writer` 执行 `record-correction`。
...
- 首页说明 “5 个索引文件 + spec.md + changes 详情 + agent writer”。
...
- `agent-references/artifact-writer-spec.md` §5.6.4 的 `findings.md` 整文件模板仍包含 `## 未解决问题`。这与 v6 “finding 详情只在 `changes/findings/`” 冲突，应删除该段或改成仅说明历史归档区可能存在。
...
- `agent-references/instructions/update-chg.md` 把已批准写成 `already-approved`。当前 agent prompt 的失败原因枚举不包含它，guidebook 建议把它作为成功幂等状态；spec/instruction 需要同步。
...
- plugin 发布包必须包含 `agents/artifact-writer.md` 与 `agent-references/**`，否则插件安装后没有 artifact writer。
...
| P0 | `agent-references/artifact-writer-spec.md` | `findings.md` 模板残留 `## 未解决问题`，VERIFIED 位置未定义，`already-approved` 需与失败枚举对齐 |
...
| P1 | `internal/skills/audit/*` | 已移出发布面；仅保留为本仓内部审计资料，审计范围需继续覆盖 agents/tests/迁移/v6 检查 |
...
3. Hook 不给 AI v5 修复提示；hook 输出必须说清楚“派 artifact-writer”。
