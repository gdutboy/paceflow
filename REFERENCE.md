# PACEflow v5.1.2 功能与状态全景参考手册

> **最后更新**：2026-03-14 | **版本**：v5.1.2
...
│   ├── pre-tool-use.js                          #   PreToolUse：三级触发+批准检查
...
- 归档操作采用标记上移：内容不动，将 `<!-- ARCHIVE -->` 标记上移到待归档内容上方（两步 Edit：插入新标记 → 删除旧标记）
...
| `--force` | 覆盖已存在的生产文件 |
| `--plugin` | 安装到本地 plugin 缓存（`~/.claude/plugins/marketplaces/`） |
| `--migrate` | 清理旧版手动安装（旧 settings.json hooks + 旧 skills 目录）|
| 无参数 | 仅安装缺失文件，跳过已存在的 |

**三步安装**：
1. hooks → `~/.claude/hooks/pace/`（脚本 + 模板）
2. skills → `~/.claude/skills/<name>/SKILL.md`（Skill 定义 + 模板）
