...
**归档必须原子化**：单次 `Edit` 操作，`old_string` 跨越被归档内容和 `<!-- ARCHIVE -->` 标记，`new_string` 把内容移到标记之后。**禁止拆分为两次 Edit**（先删后插会丢数据）。
...
4. **追加详情** — 在 `## 活跃变更详情` 区追加 `### CHG-ID 标题` 段落（**背景（Why）** + **范围（What）** + **技术决策（How）** + **T-NNN 任务标题**）
5. **随进度更新** — 批准后 `[/]`，完成后 `[x]`
