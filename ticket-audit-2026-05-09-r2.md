...
- **[W-D5] `plugin/skills/pace-knowledge/SKILL.md:142`** — "在 finding 详情正文需要补 knowledge 反向链接时记录为 artifact writer 后续更新需求"措辞模糊，主 session 无法操作 agent backlog。→ 改为"派 `update-chg action=replace` 在 finding 详情正文 `## 关联调研` 段以 wikilink 反向引用 knowledge 笔记"。
