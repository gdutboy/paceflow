...
- 子流程：
  ...
  3. **frontmatter 联动**（每次 update-status 后必执行）：
     - Read `## 任务清单` 段，统计任务状态
     - 全部为 `[-]` → Edit frontmatter `status` → `cancelled`
     - 全部为 `[x]` 或 `[-]` 且至少一个 `[x]` → Edit frontmatter `status` → `completed`（v7 帧无 `completed-date` 字段，完成时刻由 `verified-date`/`reviewed-date` 承载）
...
5. **写入**（按顺序，原子语义）：
   - Edit frontmatter：`verified-date: <date '+%Y-%m-%dT%H:%M:%S+08:00' 输出>`——只改既有 `verified-date: null` 行的值（7.0 帧该 key 恒在，位于 `change-set-seq` 与 `reviewed-date` 之间，见 spec §2.1 固定顺序）
...
- target 解析的详情文件名与 target ID 不匹配 → `id-mismatch`（7.0 帧无 chg-id 字段，ID 由文件名承载）
