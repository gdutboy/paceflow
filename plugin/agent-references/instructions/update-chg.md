...
- 仅适用于 `section=tasks`
- `new-status=[!]` 表示暂停/阻塞，必须带 `status-reason`；原因写入 `## 工作记录`。`[!]` 的语义仅为「该任务当前挂起、等待同一 owner 后续恢复」（与完成态 `[x]` 区分，且不触发其他 worktree 接手）。
- 子流程：
...
5. **写入**（按顺序，原子语义）：
   - Edit frontmatter：`verified-date: <date '+%Y-%m-%dT%H:%M:%S+08:00' 输出>`——只改既有 `verified-date: null` 行的值（该 key 恒在，位于 `change-set-seq` 与 `reviewed-date` 之间，见 spec §2.1 固定顺序）
...
- target 解析的详情文件名与 target ID 不匹配 → `id-mismatch`（ID 由文件名承载）
