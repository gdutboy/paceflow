...
- `related-changes`（可选，wikilink list——写入索引行 `[change::]` meta，7.0 帧无此字段）
- `merges`（可选，wikilink list——写入索引行 `[merges::]` meta，7.0 帧无此字段）
- `status`（默认 `open`，可选 `investigating` / `accepted` / `rejected` / `merged` / `blocked`）
- `rejection-reason`（status=rejected 时必填，≥ 10 字符——写入正文末尾「拒绝理由」段，7.0 帧无此字段）

> **7.0 帧只有 `status` / `date` / `schema-version` 三个 key**（spec §2.2 封闭合同）；`title`/`summary`/`type`/`impact` 等输入进入索引行与正文，不进 frontmatter。
