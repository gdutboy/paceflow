...
3. 使用 prompt 中的 hook 预留 `reserved-id` 作为 correction-id（CORRECTION-YYYY-MM-DD-NN）。若 prompt 缺 `reserved-id` / `reserved-file-prefix`，报告 `hook-deny` 并让主 session 重派；不得自行扫描 `changes/corrections/` 分配同日序号
