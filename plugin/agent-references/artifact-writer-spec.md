...
`changes/` 根目录是 v6 项目 marker，必须预先存在。base `changes/` 由项目初始化提供，agent 以它的存在作为执行前提；缺失时立即报告 `not-pace-project` 并停止（此时不 Write / Edit 任何 artifact）。
...
**v7 封闭合同（hook `validateFrontmatterSchema` 确定性校验）**：上方 9 个 key 是 CHG 帧的完整集合——**缺失任一 key 或出现集合外 key 都报 `format-violation`**。字段恒在、未到阶段值为 `null`；不再有 `chg-id`（ID 由文件名唯一承载）/`type`（由文件名前缀判定）/`completed-date`（完成时刻由 verified/reviewed-date 承载）/`aliases`/`tags`/`related-finding`（关联走正文 wikilink）/`parent-impl`（implementation_plan 已退役）。`change-set`/`change-set-seq` **key 恒在**，batch 成员只改值不插行——6.0 时代「单 CHG 省略这两行」的写法在 7.0 下是 missing-key 非法（CHG-09 审计 P1 裁定）。
...
**v7 封闭合同**：finding 帧只剩上方 3 个 key（缺失/多余都报 `format-violation`）。`impact`/`summary` 的权威在 findings.md 索引行（注入只读索引行）；`type`（输入枚举 `research | observation | comparison | bug-report`，不含 correction——纠正记录走 `record-correction`）由正文与 `#finding` hashtag 承载；`related-changes`/`merges`/`merged-by`/`rejection-reason` 家族删除（合并机制有 schema 无实例，rejection 理由写正文）；`finding-id` 由文件名承载。
...
**v7 封闭合同**：correction 帧只剩上方 2 个 key（缺失/多余都报 `format-violation`）。五个文本字段（`trigger-quote` / `wrong-behavior` / `correct-behavior` / `trigger-scenario` / `root-cause`）**正文 6 段单源**（见 `instructions/record-correction.md` 详情文件结构），不再进 frontmatter；`knowledge-link`/`project-scope` 由 corrections.md 索引行 `[knowledge::]`/`[scope::]` 与正文承载；`correction-id` 由文件名承载（详情文件名和 wikilink 格式 `changes/corrections/correction-yyyy-mm-dd-nn-slug.md` 不变）。
...
### 5.2 implementation_plan.md（v7 退役）

v7 起 task.md 是唯一 CHG 索引，artifact-writer 不再写 implementation_plan.md（存量由 migrate-v7 改写为 tombstone）。
...
### 5.6.2 implementation_plan.md（v7 退役）

v7 起不再创建该文件（task.md 是唯一索引）；存量项目由 migrate-v7 改写为 tombstone。
...
| 索引存在 | 同一 CHG-ID 在 task.md 有唯一索引行（v7 单索引） |
| ARCHIVE 标记 | 仅 1 个，独占一行 |
| schema 合同 | 7.0 帧 key 集合与阶段必填符合 §2.1-2.3（hook validateFrontmatterSchema 同步校验） |
