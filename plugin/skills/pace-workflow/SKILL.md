...
主 session 先预留编号，再组织字段派 artifact writer：

```bash
node "${CLAUDE_PLUGIN_ROOT}/hooks/reserve-artifact-id.js" --operation create-chg
```

将 helper 输出的 `artifact_dir` / `operation` / `reserved-id` / `reserved-file` 原样放在 Agent prompt 顶部，再追加：
...
若没有先运行 helper，`create-chg` 首次派遣仍可能被 hook 阻止并返回 `reserved-id` / `reserved-file`。这是并发保护 fallback；把这两个字段原样加入 Agent prompt 后重派，不要让 agent 自行扫描索引分配编号。
