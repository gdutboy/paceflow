...
检测到旧 v5 artifact（有 `task.md` / `implementation_plan.md` 活跃内容但无 `changes/`）时，先按 hook 提示执行 dry-run 迁移或桥接为 CHG；`changes/` 在迁移确认后才创建，迁移本身作为迁移动作单独说明、与原代码任务完成分开汇报。迁移或桥接完成后，再重试被阻止的原始写代码动作。
