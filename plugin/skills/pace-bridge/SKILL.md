...
把 helper 输出的 `artifact_dir` / `operation` / `reserved-id` / `reserved-file` 放到 Agent prompt 顶部，再追加：
...
若没有先运行 helper，hook 仍会用 reserved-id required 的 deny 作为 fallback。把 deny 文案中的 `reserved-id` / `reserved-file` 原样加入 Agent prompt 后重派；不要让 agent 自行扫描索引分配编号。
