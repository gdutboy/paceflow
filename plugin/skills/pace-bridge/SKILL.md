...
- 若用户已经明确选择 artifact root，但配置尚未写入，正确做法是先运行 hook 提示的 `set-artifact-root` helper（`--choice vault` 或 `--choice local`），再从目标项目 cwd 运行 reserve helper。禁止手写 `.pace/artifact-root`，尤其不要在 git worktree 分支目录里手写该文件；helper 只接受自身文档列出的参数，自动化只可用 `--cwd`。
