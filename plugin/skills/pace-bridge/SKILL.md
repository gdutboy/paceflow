...
- Helper 命令来源：正确做法是优先使用 SessionStart / PreToolUse 提示中的完整命令。若当前上下文没有完整 helper 命令，以当前 skill base directory 为基准拼成同版本绝对路径：`../../hooks/set-artifact-root.js`、`../../hooks/reserve-artifact-id.js` 与 `../../hooks/sync-plan.js`。若无法确定 skill base directory，先触发/等待 hook 提供 helper 命令。禁止搜索 `~/.claude/plugins/cache` 猜版本。
