...
- 若用户已经明确选择 artifact root，但 `.pace/artifact-root` 尚未写入，先写入纯文本 `vault` 或 `local`，再从目标项目 cwd 运行 reserve helper；不要给 helper 传 `--artifact-dir` / `--artifact-root` / `--project-dir`，自动化只可用 helper 的 `--cwd`。
...
每个 CHG 派遣前先预留编号。优先使用 SessionStart / PreToolUse 提示中的 reserve helper 完整命令；不要搜索 `~/.claude/plugins/cache` 猜版本。
...
优先使用 SessionStart / PreToolUse 提示中的 plan 同步 helper 完整命令；不要搜索 plugin cache 猜路径。
