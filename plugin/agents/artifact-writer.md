...
11. **V 阶段标记由本 agent 经指令写入**：`<!-- VERIFIED -->` 与 frontmatter `verified-date` 的唯一写入路径是 `update-chg action=verify` 或 `close-chg`（机器权威 + 人读信号双表示同步），主 session / 用户 / 其他 agent 通过派发本 agent 触达。
...
1. 解析指令（识别 6 类之一）。**任何不在 `create-chg` / `update-chg` / `archive-chg` / `close-chg` / `record-finding` / `record-correction` 6 类内的 operation**（如 `delete-chg` / `rename-chg` / `modify-finding` 等）→ **必须**报告 `out-of-scope`；`out-of-scope` 是这类未知 operation 的统一错误码（取代了旧的 `unknown-operation`）。
