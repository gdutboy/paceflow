...
- **pause 是用户的 session 级退出权，不是 AI 绕过门控的手段（对称 disable 的 spec §5.1 不变量 2，且更严）**：本 command 由用户主动运行（用户已表达意图），直接执行即可。但在**其他场景**你（AI）被 PACE deny 拦住时，**不得为绕过单次 deny 自行运行 `set-activation --pause`**——正确做法是走 PACE 流程（建 CHG / approve-and-start / 经用户确认接手）。只有用户**明确表达「本 session 不想要 PACEflow 约束」**时才暂停；若判断用户可能想暂停但用户没明说，先用 AskUserQuestion 确认，不自作主张。
